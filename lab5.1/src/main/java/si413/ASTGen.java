...
public class ASTGen {
    ...
    private class ExprVisitor extends Visitor<Expr> {
        ...
        @Override
        public Expr visitAskNumber(ParseRules.AskNumberContext ctx) {
            return new Expr.Number( new Expr.Input() );
        }


        @Override
        public Expr visitMulDivNumber(ParseRules.MulDivNumberContext ctx) {
            return new Expr.Number( new Expr.MathBinOp(ctx.MUL_DIV().getText(), visit(ctx.expr(0)), visit(ctx.expr(1))) );
        }


        @Override
        public Expr visitAddMinusNumber(ParseRules.AddMinusNumberContext ctx) {
            return new Expr.Number( new Expr.MathBinOp(ctx.ADD_MINUS().getText(), visit(ctx.expr(0)), visit(ctx.expr(1))) );
        }


        @Override
        public Expr visitNumberLit(ParseRules.NumberLitContext ctx) {
            return new Expr.NumberLit( ctx.NUMBER().getText() );
        }

        @Override 
        public Expr visitNumberBoolOp(ParseRules.NumberBoolOpContext ctx) {
            return new Expr.Number( new Expr.MathBinOp(ctx.NUMBOOLOPS().getText(), visit(ctx.expr(0)), visit(ctx.expr(1))) );
        }

        @Override
        public Expr visitNegateNumber(ParseRules.NegateNumberContext ctx) {
            return new Expr.Negate( visit(ctx.expr()) );
        }
    }
